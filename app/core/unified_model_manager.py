...
class UnifiedModelManager:
    ...
    def __init__(self):
        ...
        if self.backend in ["transformers", "auto"] and TRANSFORMERS_AVAILABLE:
            try:
                ...
                if self.backend == "transformers" or (self.backend == "auto" and not LLAMA_CPP_AVAILABLE):
        ...
        if not self.active_backend:
            raise ImportError("사용 가능한 모델 백엔드가 없습니다. llama-cpp-python 또는 transformers를 설치하세요.")
    ...
    async def load_model(self, model_name: str, config: Optional[ModelConfig] = None, 
        ...
        async with self.lock:
            ...
            if backend:
                ...
                elif "/" in model_name or model_name in settings.TRANSFORMERS_MODELS:
                    use_backend = "transformers"
                else:
                    use_backend = self.active_backend
            
            # 모델 ID 처리 (TRANSFORMERS_MODELS에서 참조된 경우)
            actual_model_name = model_name
            if use_backend == "transformers" and model_name in settings.TRANSFORMERS_MODELS:
    ...
    async def unload_model(self, model_name: str, backend: Optional[Literal["llama-cpp", "transformers"]] = None) -> None:
        """모델을 언로드합니다."""
        async with self.lock:
            # 백엔드 결정
            if backend:
